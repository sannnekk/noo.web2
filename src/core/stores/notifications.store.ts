...
import { useApiRequest, type UseApiRequestReturn } from '../composables/useApiRequest'
...
const useNotificationsStore = defineStore(
  ...
  (): NotificationsStore => {
    ...
    const readNotifications = useApiRequest(
      NotificationService.getRead
    )

    const unreadNotifications = useApiRequest(
      NotificationService.getUnread
    )

    const unreadCount = computed(() => readNotifications.data.value?.length ?? 0)

    const unreadCountText = computed(() => unreadCount.value > 99 ? '99+' : `${unreadCount.value}`)
